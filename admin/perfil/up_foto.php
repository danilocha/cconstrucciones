...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($archivo !='') {
		...
		if ($extension == "png" || $extension == "PNG" || $extension == "JPG" || $extension == "jpg" || $extension == "jpeg" || $extension == "JPEG") {
			...
			if (up) {
				$_SESSION['foto']  = $ruta;
				header('location:../extend/alerta.php?msj=se ha actualizado su foto con exito&c=pe&p=in&t=success');
			}else{
				header('location:../extend/alerta.php?msj=su foto de perfil no pudo ser actualizada&c=pe&p=in&t=error');
			}
		}else{
