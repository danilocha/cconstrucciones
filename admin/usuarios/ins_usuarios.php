...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (empty($nick) || empty($pass1) || empty($nivel) || empty($nombre)) {
		...
		header('location:../extend/alerta.php?msj=El nick solo debe contener letras&c=us&p=in&t=error');
		exit;
	}if (!ctype_alpha($nivel)) {
		header('location:../extend/alerta.php?msj=El nivel solo debe contener letras&c=us&p=in&t=error');
		exit;
	}
